...
class justg_API {
	...
	private function populate_accounts() {
		...
		if ( $this->accounts ) {
			uasort( $this->accounts, 'justg_sort_by_priority' );
		}
	}
	...
	private function populate_couriers() {
		...
		if ( $this->couriers ) {
			uasort( $this->couriers, 'justg_sort_by_priority' );
		}
	}
	...
	public function api_response_parser( $response ) {
		try {
			...
			if ( empty( $body ) ) {
				throw new Exception( __( 'API response is empty.', 'justg' ) );
			}
			...
			if ( $json_error && strtolower( $json_error ) !== 'no error' ) {
				...
				throw new Exception( wp_sprintf( __( 'Failed to decode the JSON data: Error: %1$s, Body: %2$s', 'justg' ), $json_error, $body ) );
			}
			...
			if ( isset( $json_data['rajaongkir']['status'] ) && 200 !== intval( $json_data['rajaongkir']['status']['code'] ) ) {
				...
				throw new Exception( wp_sprintf( __( 'Code: %1$s, Description: %2$s', 'justg' ), $error_code, $error_description ) );
			}
		...
		try {
			...
			foreach ( array_chunk( $couriers, 7 ) as $couriers_chunk ) {
				...
				if ( is_wp_error( $parsed_response ) ) {
					throw new Exception( $parsed_response->get_error_message() );
				}

				if ( ! $parsed_response || empty( $parsed_response['results'] ) ) {
					throw new Exception( __( 'API response is empty.', 'justg' ) );
				}
			...
			return new WP_Error( 'calculate_shipping_error', $e->getMessage() );
		}
	}
	...
	private function parse_etd( $etd ) {
		...
		if ( false !== strpos( $etd, 'jam' ) ) {
			...
			$etd = is_numeric( $etd ) && intval( $etd ) === 1 ? __( '1 hour', 'justg' ) : sprintf( __( '%s hours', 'justg' ), $etd );
		} else {
			$etd = trim( str_replace( 'hari', '', $etd ) );

			// translators: %s is number of days.
			$etd = is_numeric( $etd ) && intval( $etd ) === 1 ? __( '1 day', 'justg' ) : sprintf( __( '%s days', 'justg' ), $etd );
		}
